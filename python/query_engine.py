...
class QueryEngine:
    ...
    def query(self, options: QueryOptions) -> QueryResult:
        ...
        for type_name in types:
            type_def = self._store._schema.types.get(type_name)
            if type_def is None:
                continue
    ...
    def _apply_filters(
        self, type_name: str, seed: set[str], filters: list[FieldFilter], logic: str
    ) -> set[str]:
        if not filters:
            return seed
        if logic == "AND":
    ...
    def _apply_keywords(self, type_name: str, seed: set[str], keywords) -> set[str]:
        if not keywords:
            return seed
        ti = self._store.get_indexes(type_name)
        if ti is None:
...
def _resolve_types(type_opt, all_types: list[str]) -> list[str]:
    if type_opt is None:
        return all_types
...
def _normalise_filters(where) -> list[FieldFilter]:
    if not where:
        return []
