...
use crate::schema::{hash_schema, validate_schema}; // validate_record_data removed (unused)
...
impl VaporInstance {
    ...
    pub fn restore(&self, snapshot: &VaporSnapshot) -> VaporResult<VaporInstance> {
        ...
        for record in &sorted {
            let new_id = fresh
                .records
                .store(&record.record_type, record.data.clone())?;
        ...
        for edge in &snapshot.relationships {
            ...
            if let (Some(ns), Some(nt)) = (new_source, new_target) {
                ...
                let tgt_type = fresh
                    ...
                    .map(|r| r.record_type.clone())
                    .unwrap_or_default();
                let mut clean = edge.metadata.clone();
