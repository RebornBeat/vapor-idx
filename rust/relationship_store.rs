...
impl RelationshipStore {
    ...
    pub fn relate(
        ...
        let type_def = self.schema.types.get(source_type)
            .ok_or_else(|| VaporError::NotFound(format!("Type \"{source_type}\" not found")))?;

        let rel_def = type_def.relationships.get(rel_type)
            .ok_or_else(|| VaporError::Schema(format!(
                "Relationship \"{rel_type}\" not declared on type \"{source_type}\"."
            )))?;

        if !rel_def.target_types.iter().any(|t| t == "*" || t == target_type) {
            return Err(VaporError::Schema(format!(
                "Relationship \"{rel_type}\" from \"{source_type}\" does not allow target \"{target_type}\"."
            )));
        }
        ...
        self.by_type.entry(rel_type.to_owned()).or_default().insert(edge_id.clone());
        ...
        if !directed {
            ...
            self.by_type.entry(rel_type.to_owned()).or_default().insert(rev_id);
        }
    ...
    pub fn unrelate(&mut self, edge_id: &str) {
        if let Some(edge) = self.edges.remove(edge_id) {
            self.outgoing.get_mut(&edge.source_id)
                .and_then(|m| m.get_mut(&edge.relationship_type))
                .map(|s| s.remove(edge_id));
            self.incoming.get_mut(&edge.target_id)
                .and_then(|m| m.get_mut(&edge.relationship_type))
                .map(|s| s.remove(edge_id));
            self.by_type.get_mut(&edge.relationship_type)
                .map(|s| s.remove(edge_id));
        }
    }
    ...
    pub fn remove_for_record(&mut self, record_id: &str) {
        ...
        if let Some(out_map) = self.outgoing.get(record_id) {
            for ids in out_map.values() { to_remove.extend(ids.iter().cloned()); }
        }
        if let Some(in_map) = self.incoming.get(record_id) {
            for ids in in_map.values() { to_remove.extend(ids.iter().cloned()); }
        }
        let to_remove: Vec<String> = to_remove.into_iter().collect::<std::collections::HashSet<_>>().into_iter().collect();
        for eid in to_remove { self.unrelate(&eid); }
    }
    ...
    pub fn get_edges_for_record(
        ...
        if direction == "outgoing" || direction == "both" {
            if let Some(out) = self.outgoing.get(record_id) {
                match rel_type {
                    Some(t) => { out.get(t).map(|s| ids.extend(s.iter().cloned())); }
                    None    => { for s in out.values() { ids.extend(s.iter().cloned()); } }
                }
            }
        }
        if direction == "incoming" || direction == "both" {
            if let Some(inc) = self.incoming.get(record_id) {
                match rel_type {
                    Some(t) => { inc.get(t).map(|s| ids.extend(s.iter().cloned())); }
                    None    => { for s in inc.values() { ids.extend(s.iter().cloned()); } }
                }
            }
        }
    ...
    pub fn get_neighbour_ids(&self, record_id: &str, rel_type: &str, direction: &str) -> Vec<String> {
        self.get_edges_for_record(record_id, Some(rel_type), direction)
            .into_iter()
            .map(|e| if e.source_id == record_id { e.target_id } else { e.source_id })
            .collect()
    }
    ...
    fn enforce_cardinality(
        ...
        if *cardinality == Cardinality::ManyToMany { return Ok(()); }

        let out_count = self.outgoing.get(source_id)
            .and_then(|m| m.get(rel_type))
            .map(|s| s.len())
            .unwrap_or(0);

        if *cardinality == Cardinality::OneToOne && out_count > 0 {
            return Err(VaporError::Cardinality(
                format!("\"{rel_type}\" is one-to-one but source already has outgoing edge.")
            ));
        }
        if *cardinality == Cardinality::OneToOne {
            let in_count = self.incoming.get(target_id)
                .and_then(|m| m.get(rel_type))
                .map(|s| s.len())
                .unwrap_or(0);
            if in_count > 0 {
                return Err(VaporError::Cardinality(
                    format!("\"{rel_type}\" is one-to-one but target already has incoming edge.")
                ));
            }
        }
        Ok(())
    ...
    fn add_adjacency(&mut self, source: &str, target: &str, rel_type: &str, edge_id: &str) {
        self.outgoing.entry(source.to_owned()).or_default()
            .entry(rel_type.to_owned()).or_default()
            .insert(edge_id.to_owned());
        self.incoming.entry(target.to_owned()).or_default()
            .entry(rel_type.to_owned()).or_default()
            .insert(edge_id.to_owned());
    }
